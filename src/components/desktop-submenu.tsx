...
function DesktopSubmenu({
    ...
    if (navItemId === "home") {
        const navItemObj = navLinks.filter((item) => item.id === navItemId);
        return (
            <div className="p-8 flex *:basis-1/3 gap-4">
                ...
                <ul className="list-none space-y-4 pt-1">
                    {navItemObj[0].subNav?.map((item) => {
                        //const hasSubMenu = navItemObj[0].subNav && navItemObj[0].subNav.length > 0;
                        return (
                            <li className="flex gap-1" key={item.nav}>
                                <Link href={item.path} onClick={() => setNavItem(null)}>
                                    {item.nav}
                                </Link>
                                {item.subSubNav && <ChevronRightIcon className="stroke-orange-500 ml-2" />}
                                {item.subSubNav && (
                                    <ol className="list-none pl-4 space-y-1">
                                        {item.subSubNav.map((item) => (
                                            <li key={item.nav}>
                                                <Link href={item.path} onClick={() => setNavItem(null)}>
                                                    {item.nav}
                                                </Link>
